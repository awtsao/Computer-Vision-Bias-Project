...
def _test(loader):
    ...
    accuracy_sum_prob_wo_prior_shift = compute_accuracy_sum_prob_wo_prior_shift(outputs, targets)
    accuracy_sum_prob_w_prior_shift = compute_accuracy_sum_prob_w_prior_shift(outputs, targets)
    accuracy_max_prob_w_prior_shift = compute_accuracy_max_prob_w_prior_shift(outputs, targets)
    test_result = {
...
def test():
    ...
    utils.write_info(os.path.join(save_path, 'test_result.txt'), info)
...
if __name__ == '__main__':
  ...
  for x in os.listdir(path):
      if x.endswith('.pth'):
        ...
        model = cifar_model.CifarDiscriminative(os.path.join(path,x), testing=True)
